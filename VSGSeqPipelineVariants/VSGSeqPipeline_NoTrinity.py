...
if path != '':
	fullmultopath = path + '/MULTo1.0-open'
else:
...
#	for file in filebasenames:
#		subprocess.call(['cat '+header + "/"+header+"_"+file+"_orf_VSGs.fa >> " +header + "/"+header+"_orf_VSGs.fa"], shell=True) # concatenates all the files for 
#MULTo
#	
#	stderr_cd = " > " + header + "/StandardError/cdhitest.txt"
#	subprocess.call(['cd-hit-est -i '+header+"/"+header+'_orf_VSGs.fa '+' -o '+header+"/"+header+'_orf_VSGs_merged.fa -d 0 -c ' + seqIdenThresh + ' -n 8 -G 1 -g 1 -s 
#0.0 -aL 0.0 -M '+str(max_memory_cdhit)+' -T ' + numCPU + stderr_cd], shell=True)
#	
